...
<div class="container section">
  ...
  <c:if test="${param.cancelled == '1'}">
    <div class="alert alert-success" style="font-size:15px;padding:16px 20px;">
      <i class="fas fa-check-circle" style="font-size:18px;"></i>
      Đơn hàng đã được <strong>hủy thành công</strong>.
    </div>
  </c:if>
  <c:if test="${param.cancelError == '1'}">
    <div class="alert alert-danger" style="font-size:15px;padding:16px 20px;">
      <i class="fas fa-exclamation-circle" style="font-size:18px;"></i>
      Không thể hủy đơn hàng. Chỉ có thể hủy đơn hàng đang ở trạng thái <strong>Chờ xử lý</strong>.
    </div>
  </c:if>
          ...
          <c:if test="${ORDER.status == 'pending'}">
            ...
            <form action="MainController" method="post"
              ...
              <button type="submit" class="btn btn-sm"
                      style="width:100%;background:#dc3545;color:#fff;border:none;cursor:pointer;">
                Hủy đơn hàng
              </button>
            </form>
          </c:if>
